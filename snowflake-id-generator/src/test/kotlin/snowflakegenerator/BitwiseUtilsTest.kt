...
class BitwiseUtilsTest {
	...
	class Value5Bits {
		...
		fun `should throw exception when startBit is out of range`() {
			...
			}.isExactlyInstanceOf(IllegalArgumentException::class.java).hasMessage(
				"Starting bit must match range 0..${LENGTH_5_BITS - 1}, " + "otherwise it will be out of bounds.",
			)
		}
		...
		fun `should throw exception when bitCount is out of range`() {
			assertThatCode {
				...
				"Count of bits to be extracted " + "must match range of a given number that is 1..$LENGTH_5_BITS.",
			)
		}
