...
class GeneratorTest {
	...
	fun `Should fail to init Generator when given Datacenter ID doesn't match range`(invalidDatacenterId: Long) {
		...
		}.isExactlyInstanceOf(IllegalArgumentException::class.java).hasMessage("Datacenter ID must match range 1 .. 31")
	}
	...
	fun `Should fail to instantiate Generator when given Worker ID doesn't match possible range`(invalidWorkerId: Long) {
		...
		}.isExactlyInstanceOf(IllegalArgumentException::class.java).hasMessage("Worker ID must match range 1 .. 31")
	}
	...
	fun `Should fail to instantiate Generator when given Sequence doesn't match possible range`(invalidSequence: Long) {
		...
		}.isExactlyInstanceOf(IllegalArgumentException::class.java).hasMessage("Sequence must match range 0 .. 4095")
	}
	...
	fun `Should generate deterministic identifier based on given parameters`() =
		runTest {
			...
			assertThat(actualResult).describedAs("Generated ID should be deterministic based on given parameters.")
	...
	fun `Should generate unique deterministic id based on given parameters when call multiple times`() =
		runTest {
			...
			assertThat(actualResult1).describedAs("Generated ID [Attempt #1] should be deterministic based on given parameters.")
			...
			assertThat(actualResult2).describedAs("Generated ID [Attempt #2] should be deterministic based on given parameters.")
	...
	fun `Should fail due to timeout while generating new timestamp when sequence exceeded for same timestamp`() {
		...
		}.isExactlyInstanceOf(TimeoutCancellationException::class.java).hasMessageStartingWith("Timed out after 3s of")
	}
